...
//       console.log("for check cart userRef", userRef);
//       console.log("for check cart productRef", productRef);
//       const inventoryRef = selectedColor || selectedLevel
//       const { data } = await getCartProducts(userRef, '', inventoryRef, productRef);
//       setCartProducts(data?.cartDetails);
//     };

//     fetchCartProducts();
//   }, [ ]);
//   // selectedLevel,selectedColor, productRef

//   console.log("for inventoryRef, productRef", inventoryRef, productRef);
//   console.log("for check cart selectedLevel, setSelectedColor", selectedLevel, selectedColor);
//   console.log("for check cart products", cartProducts);

//   const handleAddToCart = async () => {
//     try {
//       console.log("for check inventoryRef", inventoryRef);
//       const user = await getUser();
...
//   if (inventoryType === "inventory") {
//     isDisabled = inventoryRef ? inventoryRef?.[0]?.availableQuantity < count : true;
//     // setStock(inventoryRef?.[0]?.availableQuantity);

//   } else if (inventoryType === "levelInventory") {
//     const match = inventoryRef ? inventoryRef?.find(item => item._id === selectedLevel) : null;
//     isDisabled = !match || match.availableQuantity < count;
//     // setStock(match?.availableQuantity);

//   } else if (inventoryType === "colorInventory") {
//     const match = inventoryRef ?  inventoryRef?.find(
//       item => item._id === selectedColor
//     ) : null;
//     isDisabled = !match || match.availableQuantity < count;
//     // setStock(match?.availableQuantity);
//   } else if (inventoryType === "colorLevelInventory") {
//     const match = inventoryRef ? inventoryRef?.find(item => item._id === selectedColor) : null;
//     isDisabled = !match || match.availableQuantity < count;
//     // setStock(match?.availableQuantity);
//   }
...
//                               setLevelError(false);
//                             }}
//                             className={`p-1 border border-[#262626]/60 hover:bg-[#1F4193] hover:text-[#fff] duration-300 cursor-pointer rounded text-center flex items-center justify-center uppercase ${
//                               level === size.level
...
//               >
//                 <button
//                 className={` flex items-center justify-center gap-1 px-4 py-3 font-semibold text-[12px] rounded text-[#fff]  ${isDisabled ? "cursor-not-allowed" : "cursor-pointer"}`}>
//                   <span>
//                     <FiPlus />
//                   </span>
...
//                 const fallbackItem =Array.isArray(inventoryRef) && (inventoryRef ? inventoryRef[0] : null);
//                 const price = selectedItem?.price ?? (fallbackItem && 'price' in fallbackItem ? fallbackItem.price : undefined);
